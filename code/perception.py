...
def color_thresh2(img, rgb_lower=(0, 0, 0), rgb_upper=(255, 255, 255)):
    return cv2.inRange(img, rgb_lower, rgb_upper)
...
def perception_step(Rover):
    ...
    :type Rover: drive_rover.RoverState
    :return:
    """
    ...
    navigable = color_thresh2(warped, rgb_lower=(160, 160, 160))
    obstacle = color_thresh2(warped, rgb_upper=(160, 160, 160))
    rock_sample = color_thresh2(warped, rgb_lower=(100, 110, 0), rgb_upper=(200, 160, 60))
    ...
    rock_x_rover, rock_y_rover = rover_coords(rock_sample)
    ...
    if rock_x_rover.any():
        Rover._saw_rock = True
        dist, angles = to_polar_coords(rock_x_rover, rock_y_rover)
        print("=" * 20, np.min(dist), np.mean(angles * 180 / np.pi))
    else:
        Rover._saw_rock = False
